(() => {
  ...
  async function restoreDocument(doc) {
    clearSelection(true); clearReference(true); state.layers.forEach(l => l.canvas.remove()); state.width = doc.width; state.height = doc.height; Object.assign(state.timeline, { frame: 0, duration: 250, fps: 12, loop: true, onion: false }, doc.timeline || {});
  ...
  $('new-file').onclick=()=>{if(confirm('Start a new blank canvas? Unsaved work will be lost.')){clearSelection(true);clearReference(true);state.projectFileHandle=null;state.layers.forEach(l=>l.canvas.remove());state.layers=[];state.timeline.frame=0;state.timeline.duration=250;makeLayer('Layer 1');placeCanvas();fitCanvas();commitHistory();setStatus('New 250-frame canvas created');}};
  ...
  function isTextEditingTarget(target) { return target.matches('input[type=number], input[type=text], input[type=color], input[type=file], textarea, select'); }
  document.addEventListener('keydown',e=>{const modifier=e.metaKey||e.ctrlKey;if(modifier&&e.key.toLowerCase()==='s'){e.preventDefault();$('save-project').click();return;}if(modifier&&e.key.toLowerCase()==='e'){e.preventDefault();$('export-png').click();return;}if(modifier&&e.key.toLowerCase()==='z'){e.preventDefault();if(e.shiftKey)redo();else undo();return;}if(isTextEditingTarget(e.target))return;if(e.key===' '){e.preventDefault();if(e.repeat||state.spaceHeld)return;state.spaceHeld=true;state.spaceTimer=setTimeout(()=>{state.spacePan=true;wrap.classList.add('panning');},180);return;}if(e.key==='Escape'){clearSelection();return;}if(e.key==='Enter'){e.preventDefault();togglePlayback();return;}if(e.shiftKey&&e.key.toLowerCase()==='n'){$('add-layer').click();return;}if(e.shiftKey&&e.key.toLowerCase()==='m'){$('merge-layer-down').click();return;}if(e.key.toLowerCase()==='b')chooseTool('brush');if(e.key.toLowerCase()==='e')chooseTool('eraser');if(e.key.toLowerCase()==='g')chooseTool('bucket');if(e.key.toLowerCase()==='h')chooseTool('pan');if(e.key.toLowerCase()==='l')chooseTool('lasso');if(e.key.toLowerCase()==='r')chooseTool('reference');if(e.key==='[')changeBrushSize(-2);if(e.key===']')changeBrushSize(2);if(e.key==='{')changeSoftness(5);if(e.key==='}')changeSoftness(-5);if(e.key==='+'||e.key==='=')zoomAtCenter(1.2);if(e.key==='-')zoomAtCenter(1/1.2);if(e.key==='1')fitCanvas();if(e.key==='Delete'||e.key==='Backspace'){if(!deleteSelectedPixels())$('delete-layer').click();}});document.addEventListener('keyup',e=>{if(e.key!==' '||isTextEditingTarget(e.target))return;clearTimeout(state.spaceTimer);const wasPanning=state.spacePan;state.spaceHeld=false;state.spacePan=false;if(state.tool!=='pan')wrap.classList.remove('panning');if(!wasPanning)togglePlayback();});
