(() => {
  ...
  async function restoreDocument(doc) {
    clearSelection(true); clearReference(true); state.layers.forEach(l => l.canvas.remove()); state.width = doc.width; state.height = doc.height; Object.assign(state.timeline, { frame: 0, duration: 150, fps: 12, loop: true, onion: false }, doc.timeline || {});
  ...
  $('new-file').onclick=()=>{if(confirm('Start a new blank canvas? Unsaved work will be lost.')){clearSelection(true);clearReference(true);state.projectFileHandle=null;state.layers.forEach(l=>l.canvas.remove());state.layers=[];state.timeline.frame=0;state.timeline.duration=150;makeLayer('Layer 1');placeCanvas();fitCanvas();commitHistory();setStatus('New 150-frame canvas created');}};
